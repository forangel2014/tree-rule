...
public class Psi2 {
	...
	public static void main(String[] args) throws IOException {
		...
		ResultSet rs1 = new ResultSet("anyburl  ", "exp/understanding/fb237/anyburl-c3-3600-100-test", true, 100);
		ResultSet rs2 = new ResultSet("complex  ", "exp/understanding/fb237/rescal-100-test", true, 100);
		...
		for (int i = 0; i < triples.size(); i++) {
			...
			Psi2 psi = new Psi2(rs2.getCompletionResult(triple), rs1.getCompletionResult(triple));
			...
			for (int j = 0; j < 10; j++) {
				...
				if (score > 10.0 && tail.equals(tailPrediction)) {
					foundAnomalyTail = true;
				}
				if  (tail.equals(tailPrediction))  {
					sb.append("[" + j + "] HIT >>> " + df.format(score) + ": " +  t.get(tailPrediction) +  " " + explanation + "\n");
				}
				else {
	...
	public Psi2(CompletionResult kgeResult, CompletionResult abResult) {
	...
	public double getTailScore(int posInKGE) {
		...
		if ((abConfidence / abConfindeceAtPos > 13.151 ) && (abConfidence / abConfindeceAtPos < 13.154)) {
			System.out.println(">>> " + abConfidence / abConfindeceAtPos);
			System.out.println(">>> " + abConfidence);
			System.out.println(">>> " + abConfindeceAtPos);
			System.out.println(">>> " + posInKGE);
