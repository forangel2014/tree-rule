...
public class Atom {
	...
	public Atom getXYGeneralization() {
		...
		copy.setLeft("X".intern());
		copy.setLeftC(false);
		copy.setRight("Y".intern());
	...
	*/
	...
	public int replaceByVariable(String constant, String variable) {
		...
		if (this.leftC && this.left.equals(constant)) {
			this.leftC = false;
			this.left = variable.intern();
			i++;
		}
		if (this.rightC && this.right.equals(constant)) {
			this.rightC = false;
			this.right = variable.intern();
			i++;
		}
		return i;
	}
	...
	public int replace(String vOld, String vNew, int block) {
		if (this.left.equals(vOld) && block != -1) {
			this.left = vNew.intern();
			return -1;
			
		}
		if (this.right.equals(vOld) && block != 1) {
			this.right = vNew.intern();
			return 1;
		}
		return 0;
	}
	...
	/**
	 * Returns the constant in the atom. If there is no constant the return value is null.
	 * 
	 * @return Constant used in the head if there is such a constant.
	 */
	...
	/**
	* This is a weird method. It seems to exclude on purpose X andY, so the name is misleading, take care!
	* @return
	*/
