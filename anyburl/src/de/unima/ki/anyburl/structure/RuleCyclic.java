...
public class RuleCyclic extends Rule  {
	...
	public HashSet<String> computeTailResults(String head, TripleSet ts) {
		...
		this.getCyclicTree("X", "Y", head, 0, true, ts,  new HashSet<String>(), results, count);
	...
	public HashSet<String> computeHeadResults(String tail, TripleSet ts) {
		...
		this.getCyclicTree("Y", "X", tail, this.bodysize() - 1, false, ts,  new HashSet<String>(), results, count);
	...
	*/
	...
	protected void getCyclicTree(String currentVariable, String lastVariable, String value, int bodyIndex, boolean direction, TripleSet triples, HashSet<String> previousValues, HashSet<String> finalResults, Timer count) {
		
		HashMap<String, Integer> variables_cnt = new HashMap<String, Integer>();
		variables_cnt.put(this.head.getLeft(), 1);
		variables_cnt.put(this.head.getRight(), 1);
		for (Atom a : this.body){
			String L = a.getLeft();
			String R = a.getRight();
			if (variables_cnt.keySet().contains(L)) variables_cnt.put(L, variables_cnt.get(L)+1);
			else variables_cnt.put(L, 1);
			if (variables_cnt.keySet().contains(R)) variables_cnt.put(R, variables_cnt.get(R)+1);
			else variables_cnt.put(R, 1);
		}
		Set<Atom> branches = new HashSet<Atom>();
		Set<Atom> stem = new HashSet<Atom>();

		for (Atom a : this.body){
			String L = a.getLeft();
			String R = a.getRight();
			if (variables_cnt.get(L) == 1 || variables_cnt.get(R) == 1 || L == R) branches.add(a);
			else stem.add(a);
		}

		HashMap<String, Set<String>> constraints = new HashMap<String, Set<String>>();
		for (Atom branch : branches){
			String predicate = branch.getRelation();
			if (predicate == "is"){
				String constraint_variable = branch.getRight();
				Set<String> constraint_set = new HashSet<String>();
				constraint_set.add(branch.getLeft());
				constraints.put(constraint_variable, constraint_set);
			}
			if (triples.getRelations().contains(predicate)) {
				boolean headNotTail = variables_cnt.get(branch.getLeft()) == 1;
				String constraint_variable = headNotTail? branch.getRight() : branch.getLeft();
				Set<String> constraint_set = headNotTail? triples.relation2tail.get(predicate) : triples.relation2head.get(predicate);
				constraints.put(constraint_variable, constraint_set);
			}
			if (triples.type2entity.containsKey(predicate)){
				String constraint_variable = branch.getRight();
				Set<String> constraint_set = triples.type2entity.get(predicate);
				constraints.put(constraint_variable, constraint_set);
			}
		}

		HashMap<String, String> init = new HashMap<String, String>();
		init.put(currentVariable, value);
		Set<HashMap<String, String>> groundings = new HashSet<HashMap<String, String>>();
		groundings.add(init);
		Set<String> processed_variables = new HashSet<String>();
		//processed_variables.add(currentVariable);
		Set<String> current_variables =  new HashSet<String>();
		current_variables.add(currentVariable);

		while (!current_variables.isEmpty()){
			Set<String> next_variables =  new HashSet<String>();
			for (String current_variable : current_variables){
				Set<Atom> relevant_atoms = new HashSet<Atom>();
				for (Atom a : stem){
					if ((a.getLeft() == current_variable && !processed_variables.contains(a.getRight())) || (a.getRight() == current_variable && !processed_variables.contains(a.getLeft()))){
						relevant_atoms.add(a);
					}
				}
				for (Atom a : relevant_atoms){
					boolean headNotTail = a.getLeft() == current_variable;
					String next_variable = headNotTail ? a.getRight() : a.getLeft();
					next_variables.add(next_variable);
					Set<HashMap<String, String>> next_groundings = new HashSet<HashMap<String, String>>();
					for (HashMap<String, String> grounding : groundings){
						String current_value = grounding.get(current_variable);
						Set<String> candidates = triples.getEntities(a.getRelation(), current_value, headNotTail);
						for (String candidate : candidates){
							HashMap<String, String> next_grounding = new HashMap<String, String>();
							//next_grounding.putAll(grounding);  // extremely time consuming!!!!!!!!!
							next_grounding.put(next_variable, candidate);
							next_groundings.add(next_grounding);
						}
					}
					groundings = next_groundings;
				}
				processed_variables.add(current_variable);
				
				if (constraints.keySet().contains(current_variable)){
					Set<String> constraint_set = constraints.get(current_variable);
					Set<HashMap<String, String>> next_groundings = new HashSet<HashMap<String, String>>();
					for (HashMap<String, String> grounding : groundings){
						if (constraint_set.contains(grounding.get(current_variable))){
							next_groundings.add(grounding);
						}
					}
					groundings = next_groundings;
				}
		...
		Set<HashMap<String, String>> left_groundings = new HashSet<HashMap<String, String>>();
		for (HashMap<String, String> grounding : groundings){
			boolean flag = true;
			for (String variable : constraints.keySet()){
				if (!constraints.get(variable).contains(grounding.get(variable))){
					flag = false;
					break;
				}
			}
			if (flag) left_groundings.add(grounding);
		}
		groundings = left_groundings;
		*/
		for (HashMap<String, String> grounding : groundings){
			finalResults.add(grounding.get(lastVariable));
		}
		return;
	}	
