...
public class TripleSet {
	...
	// add
	public HashMap<String, Set<String>> relation2head = new HashMap<String, Set<String>>();
	public HashMap<String, Set<String>> relation2tail = new HashMap<String, Set<String>>();
	public HashMap<String, Set<String>> type2entity = new HashMap<String, Set<String>>();
	...
	public TripleSet(String filepath, String typefilepath) {
		...
		// add
		this.readTypeInfo(typefilepath);
	}
	...
	private void addTripleToIndex(Triple t) {
		...
		// add
		if (relation2head.keySet().contains(relation)){
			this.relation2head.get(relation).add(head);
			this.relation2tail.get(relation).add(tail);
		}
		else {
			this.relation2head.put(relation, new HashSet<String>());
			this.relation2tail.put(relation, new HashSet<String>());
		}
	...
	private void readTypeInfo(String filepath) {
		Path file = (new File(filepath)).toPath();
		// Charset charset = Charset.forName("US-ASCII");
		Charset charset = Charset.forName("UTF8");
		String line = null;
		...
		try (BufferedReader reader = Files.newBufferedReader(file, charset)) { 
			while ((line = reader.readLine()) != null) {
				...
				String[] token = line.split("\t");
				String type = token[0];
				String[] entities_ls = token[1].split(" ");
				Set<String> entities_set =  new HashSet<String>();
				Collections.addAll(entities_set, entities_ls);
				this.type2entity.put(type, entities_set);
			}
		}
		catch (IOException x) {
			System.err.format("IOException: %s%n", x);
